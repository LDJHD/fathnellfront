...
import { getProductImageUrl, getBanniereImageUrl, getCollectionImageUrl } from "../utils/imageUtils";

export default function Magazin() {
    const { collectionId } = useParams(); // Récupère l'ID de la collection depuis l'URL
    ...
    const [collection, setCollection] = useState(null); // Collection de la BDD
    const { isInWishlist, toggleWishlist } = useWishlist(); // Hook pour la wishlist

    // Charger la collection depuis la BDD
    ...
    // Fonction pour obtenir les informations de la collection actuelle depuis la BDD
    const getCurrentCollection = () => {
        if (collection) {
            return {
                title: collection.nom,
                description: collection.description || "Découvrez notre sélection de produits de qualité."
            };
        }
        
        // Collection par défaut si aucune n'est spécifiée
        return {
            title: "Collection",
    ...
    const fetchProduits = async (reset = false) => {
        try {
            ...
            // Filtrer par collection si spécifié dans l'URL
            if (collectionId) {
                // Utiliser l'ID de collection pour filtrer les produits
                filters.collection_id = collectionId;
            } else {
                // Si aucune collection n'est spécifiée, ne pas charger de produits
                setProduits([]);
                setLoading(false);
                setHasMore(false);
                return;
    ...
    // Recharger les produits quand la collection change
    useEffect(() => {
        if (collectionId) {
            setOffset(0);
            setHasMore(true);
        ...
        <div className="text-center w-full flex flex-col items-center bg-white">
            ...
            <div className="w-full md:w-[1000px] px-8 md:px-10 py-10">
                ...
                <p className="text-black text-xl font-normal font-['Glacial_Indifference'] leading-relaxed md:leading-10">
                    {getCurrentCollection().description}
                </p>
            </div>
